...
class IndexingCoordinator(BaseService):
    ...
    def detect_file_language(self, file_path: Path) -> Optional[Language]:
        ...
        language_map = {
            ...
            '.txt': Language.TEXT, '.rst': Language.TEXT,
            ...
            '.xml': Language.XML,
            '.toml': Language.TOML,
            '.ini': Language.INI, '.cfg': Language.INI,
    ...
    async def process_file(self, file_path: Path, skip_embeddings: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            existing_file = self._db.get_file_by_path(str(normalized_path))
            ...
            file_id = self._store_file_record(normalized_path, file_stat, language)
            if file_id is None:
            ...
            # Generate embeddings if requested and provider available
    ...
    def _store_file_record(self, file_path: Path, file_stat: Any, language: Language) -> int:
        ...
        # Ensure path is normalized (resolve symlinks)
        normalized_path = file_path.resolve()
        ...
        file_model = File(
            path=FilePath(str(normalized_path)),
    ...
    async def remove_file(self, file_path: str) -> int:
        ...
        try:
            ...
            if isinstance(file_path, Path):
                ...
                # If string path, try to resolve it
