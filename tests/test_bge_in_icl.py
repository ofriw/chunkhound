...
class TestICLContextManager:
    ...
    def test_get_context_for_unknown_language(self):
        ...
        assert (
            context["instruction"]
            == manager._language_templates["generic"]["instruction"]
        )
        assert (
            context["examples"]
            == manager._language_templates["generic"]["examples"][:2]
        )
...
class TestBGEInICLProvider:
    ...
    def test_prepare_icl_request_disabled(self):
        ...
        provider = BGEInICLProvider(base_url="http://localhost:8080", enable_icl=False)

        texts = ["def test(): pass", "class Example: pass"]
        request = provider._prepare_icl_request(texts)

        expected = {"model": "bge-in-icl", "input": texts, "encoding_format": "float"}
        assert request == expected

    def test_prepare_icl_request_enabled(self):
        """Test request preparation with ICL enabled."""
        provider = BGEInICLProvider(
            base_url="http://localhost:8080", language="python", enable_icl=True
        )
    ...
    def test_batch_preparation(self):
        ...
        provider = BGEInICLProvider(
            base_url="http://localhost:8080", batch_size=2, enable_icl=False
        )
    ...
    def test_provider_configuration(self):
        ...
        provider = BGEInICLProvider(
            base_url="http://localhost:8080", timeout=60, batch_size=32
        )
    ...
    def test_api_key_configuration(self):
        ...
        provider = BGEInICLProvider(
            base_url="http://localhost:8080", api_key="test_api_key"
        )
...
class TestBGEInICLRealWorldScenarios:
    ...
    def test_python_code_embedding_preparation(self):
        ...
        provider = BGEInICLProvider(
            base_url="http://localhost:8080", language="auto", enable_icl=True
        )
    ...
    def test_context_templates_completeness(self):
        ...
        for language, template in manager._language_templates.items():
            ...
            assert len(template["examples"]) >= 2, (
                f"Language {language} needs at least 2 examples"
            )
            assert isinstance(template["instruction"], str), (
                f"Language {language} instruction must be string"
            )
            assert len(template["instruction"]) > 10, (
                f"Language {language} instruction too short"
            )
