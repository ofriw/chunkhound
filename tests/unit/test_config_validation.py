...
class TestConfigValidation:
    ...
    def test_validate_for_index_command(self):
        ...
        config = Config(embedding={"provider": "openai", "api_key": "sk-test"})
        errors = config.validate_for_command("index")
        assert len(errors) == 0
        
        # Missing API key for OpenAI
        config = Config(embedding={"provider": "openai"})
        errors = config.validate_for_command("index")
    ...
    def test_validate_database_path(self):
        ...
        config = Config(database={"path": "/nonexistent/path/db"})
        errors = config.validate_for_command("index")
        # Should not have database path errors
        assert not any("database" in err.lower() and "path" in err.lower() for err in errors)
    ...
    def test_command_specific_requirements(self):
        ...
        config_no_embedding = Config(database={"path": "/tmp/test.db"})
        config_with_embedding = Config(
    ...
    def test_debug_mode_validation(self):
        ...
        config_normal = Config(debug=False)
    ...
    def test_validation_error_messages(self):
        ...
        config = Config(embedding={"provider": "tei"})
        errors = config.validate_for_command("index")
