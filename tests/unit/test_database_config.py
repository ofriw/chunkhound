...
class TestDatabaseConfig:
    ...
    def test_provider_validation(self):
        ...
        assert "provider" in str(exc_info.value) or "literal_error" in str(
            exc_info.value
        )
    ...
    def test_pool_settings_validation(self):
        ...
        # Valid pool settings
        config = DatabaseConfig(
            pool_size=10, max_overflow=20, cache_size=500, timeout=60
        )
    ...
    def test_config_json_serialization(self):
        ...
        config = DatabaseConfig(provider="duckdb", path="/tmp/test.db", pool_size=15)
    ...
    def test_repr(self):
        ...
        config = DatabaseConfig(provider="lancedb", path="/tmp/db", pool_size=8)
