...
class TestCLIConfigIntegration:
    ...
    def test_index_command_with_local_config(self):
        ...
        # Run index command
        cmd = [
            sys.executable, "-m", "chunkhound",
            "index", str(self.project_dir)
        ]
        ...
        assert str(config.database.path) == str(self.project_dir / ".chunkhound" / "local.db")
    ...
    def test_search_command_env_override(self):
        ...
        # Create config file
        config_data = {
            "database": {"path": str(self.temp_dir / "config.db")}
        }
...
class TestComplexProjectStructures:
    ...
    def test_monorepo_config_isolation(self):
        ...
        backend_config = {
            "indexing": {"include": ["*.py"], "exclude": ["*_test.py"]}
        }
