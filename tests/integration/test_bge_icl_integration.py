...
class BGEICLIntegrationTestSuite:
    ...
    def __init__(
        self, server_url: str = TEST_SERVER_URL, api_key: Optional[str] = TEST_API_KEY
    ):
    ...
    async def setup(self):
        ...
        logger.info(
            f"Setting up BGE-IN-ICL integration tests with server: {self.server_url}"
        )
    ...
    async def test_basic_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            assert all(len(emb) == self.provider.dims for emb in embeddings), (
                "Dimension mismatch"
            )
            ...
            logger.info(
                f"Basic test passed: {elapsed:.2f}s, {result['throughput']:.1f} emb/s"
            )
            return result
    ...
    async def test_language_specific_optimization(self) -> Dict[str, Any]:
        ...
        for language, texts in LANGUAGE_SPECIFIC_TESTS.items():
            ...
            try:
                ...
                logger.info(
                    f"{language}: {elapsed:.2f}s, cache hit: {metrics.get('cache_hit_rate', 0):.1%}"
                )
    ...
    async def test_adaptive_batching(self) -> Dict[str, Any]:
        ...
        for texts, scenario in test_scenarios:
            ...
            try:
                ...
                logger.info(
                    f"{scenario}: batch_size={current_batch_size}, time={elapsed:.2f}s"
                )
                ...
                return {"status": "fail", "error": str(e), "scenario": scenario}
        ...
        return {
            ...
            "details": "Adaptive batching shows size variation"
    ...
    async def test_performance_characteristics(self) -> Dict[str, Any]:
        ...
        for batch_size in batch_sizes:
            ...
            for run in range(3):
                ...
                try:
                    ...
                    logger.error(
                        f"Performance test failed for batch_size={batch_size}, run={run}: {e}"
                    )
            ...
            if valid_times:
                ...
                logger.info(
                    f"Batch {batch_size}: {avg_time:.2f}s avg, {throughput:.1f} emb/s"
                )
        ...
        return {
            ...
            "best_throughput": max(
                (r["throughput"] for r in performance_results.values()), default=0
            ),
        }
    ...
    async def test_cache_efficiency(self) -> Dict[str, Any]:
        ...
        return {
            "status": "pass",
            "first_request": {"time": first_time, "cache_hit_rate": first_hit_rate},
            "second_request": {"time": second_time, "cache_hit_rate": second_hit_rate},
    ...
    async def test_concurrent_requests(self) -> Dict[str, Any]:
        ...
        providers = [
            create_bge_in_icl_provider(
                base_url=self.server_url, api_key=self.api_key, batch_size=16
            )
            for _ in range(5)
        ]
