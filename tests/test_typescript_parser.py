...
def test_parser_initialization(typescript_parser):
    ...
    assert (
        typescript_parser.is_available or not typescript_parser.is_available
...
def test_typescript_interface_parsing(typescript_parser, sample_ts_code):
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".ts", delete=False) as f:
        ...
        try:
            ...
            interface_chunks = [
                c for c in result.chunks if c["chunk_type"] == "interface"
            ]
...
def test_typescript_specific_features():
    ...
    parser_file = (
        Path(__file__).parent.parent / "providers" / "parsing" / "typescript_parser.py"
    )
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".ts", delete=False) as f:
        ...
        try:
            ...
            assert len(result.errors) == 0 or all(
                "Impossible pattern" not in error for error in result.errors
            )
            assert result.total_chunks > 0
