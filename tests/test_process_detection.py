...
class TestProcessDetector:
    ...
    @patch("chunkhound.process_detection.ProcessDetector.validate_pid_active")
    @patch("psutil.Process")
    def test_find_mcp_server_not_chunkhound(
        self, mock_process_class, mock_validate, mock_is_chunkhound
    ):
    ...
    @patch("chunkhound.process_detection.ProcessDetector.validate_pid_active")
    @patch("psutil.Process")
    def test_find_mcp_server_success(
        self, mock_process_class, mock_validate, mock_is_chunkhound
    ):
    ...
    def test_is_chunkhound_mcp_different_command(self):
        ...
        mock_process = Mock()
        mock_process.pid = 12345
        mock_process.cmdline.return_value = ["python", "-m", "some_other_app", "server"]
