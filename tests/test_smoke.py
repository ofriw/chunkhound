...
class TestCLICommands:
    ...
    def test_cli_help_commands(self, command):
        ...
        assert result.stdout or result.stderr, f"Command {' '.join(command)} produced no output"
    ...
    def test_mcp_http_import(self):
        ...
        """
        result = subprocess.run(
            ["uv", "run", "python", "-c", "import chunkhound.mcp_http_server; print('OK')"],
            capture_output=True,
            text=True,
            timeout=5
        )
...
class TestServerStartup:
    ...
    async def test_mcp_http_server_starts(self):
        ...
        proc = await asyncio.create_subprocess_exec(
            "uv", "run", "chunkhound", "mcp", "--http", "--port", "0",
    ...
    async def test_mcp_stdio_server_help(self):
        ...
        proc = await asyncio.create_subprocess_exec(
            "uv", "run", "chunkhound", "mcp", "stdio", "--help",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
