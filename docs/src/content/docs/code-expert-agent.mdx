...
<Card title="First-Time Setup" icon="rocket">
  **New to ChunkHound?** Start with the [Tutorial](/tutorial/) to install, configure, and index your first codebase.
</Card>
...
<Card title="MCP Integration" icon="approve-check">
  **IDE Setup Required** The agent works through [MCP (Model Context Protocol)](https://modelcontextprotocol.io/). Your AI assistant must be configured to connect to ChunkHound as described in the [Tutorial](/tutorial/#ide-setup).
</Card>
...
The Code Expert Agent uses ChunkHound's MCP tools (`mcp__ChunkHound__search_semantic`, `mcp__ChunkHound__search_regex`) for its analysis. If these tools aren't available, check your [MCP configuration](/tutorial/#ide-setup) and ensure ChunkHound is running as an MCP server.
</Aside>

### 2. Ask Before You Code

Once ChunkHound is [indexed](/tutorial/#example-initial-index) and your MCP server is running, simply ask about your codebase before implementing:
